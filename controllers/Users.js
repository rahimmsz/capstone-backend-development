...
export const deleteUser = async (req, res) => {
  try {
    ...
    switch (user.role.toLowerCase()) {
      case "petani":
        const petani = await PetaniUsers.findOne({
        ...
        break;
      case "logistik":
        const logistik = await LogisticUser.findOne({
          where: { uuid: user.uuid },
        });
      ...
      default:
        console.log(
          "Role pengguna tidak dikenali, tidak ada foto untuk dihapus."
        );
    }
...
export const getUserById = async (req, res) => {
  try {
    ...
    switch (user.role.toLowerCase()) {
      case "petani":
        userDetails.additionalInfo = await PetaniUsers.findOne({
          where: { uuid: user.uuid },
        });
        break;
      case "logistik":
        userDetails.additionalInfo = await LogisticUser.findOne({
          where: { uuid: user.uuid },
        });
        break;
      case "pabrik":
        userDetails.additionalInfo = await PabrikUser.findOne({
          where: { uuid: user.uuid },
        });
        break;
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      if (
        userToUpdate.foto &&
        !userToUpdate.foto.startsWith("defaultProfile.png")
      ) {
    ...
    switch (role.toLowerCase()) {
      ...
      default:
        console.log(
          "Role pengguna tidak dikenali, update detail pengguna tidak dilakukan."
        );
        break;
    }
